...
const createNewRoomHandler = (data, socket) => {
  ...
  // emit an event to all users connected to that room about new users which are right now in this room
...
const disconnectHandler = (socket) => {
  // find if the user has been registered, if yes remove him from the room and connected users array
  ...
  if (user) {
    ...
    if (room.connectedUsers.length > 0) {
      // emit to all users which are still in room that the user disconnected
      io.to(room.id).emit("user-disconnected", { socketId: socket.id });

      // emit an event to rest of the user which left in the room new connectedUsers in room
